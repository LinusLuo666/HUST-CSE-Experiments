...
r4 = randi(8,300,1); %创建1 到10 之间的随机整数值数组
...
for f2=1:n
    for f1=1:m
        ...
        pi2=uint8(pi2);
        %disp(pi2);
        pi1=Double_Picture(f1,f2);
        %pi1=0;
        %disp(pi1);
        pi1=bitand(pi1,pi2)+2^(r4(p)-1)*(msg(p,1));
        %pi1=bitand(pi1,pi2)+2^(r4(p)-1);
        %disp(pi1);
        Double_Picture(f1,f2)=pi1;
        %disp(msg(p,1));
        %disp(r4(p));
        %disp(uint8(2^(r4(p)-1)));
        disp(bitand(Double_Picture(f1,f2),uint8(2^(r4(p)-1))));
        % 如果p达到了私密信息长度，跳出循环
        if p==len
            break;
        end
